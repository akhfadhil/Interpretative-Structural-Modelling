import copy
import sys
from ISMt21 import outputISM, varResiko
...
if len(sys.argv) > 1:
    ...
    for code in listCode:
        risk = {'riskCode': code, 'riskName': varResiko[code], 'listPakar': copy.deepcopy(listPakar)}
    ...
    simulated_input_pakar2 = [
        [8, 10, 2],  # Risk 1
        [9, 10, 3],  # Risk 2  [S, O, D]                            # SIMULATE INPUT SOD & LINGUISTIK
        [10, 2, 2]  # Risk 3
    ]
    ...
    for i in range(len(data_input)):
        result_FUZZY["data_sod"]["risk"+str(i+1)] = [listCode[i], varResiko[listCode[i]], data_input[0][i]+data_input[1][i]+data_input[2][i]]
    # Linguistik
